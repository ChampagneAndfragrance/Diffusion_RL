...
def train_traj_grader():
    # INFO: Initialize datasets
    train_dataset = TrajGraderDataset(file_root="./datasets/balance_game/diffusion_reward/train")
    valid_dataset = TrajGraderDataset(file_root="./datasets/balance_game/diffusion_reward/valid")
    test_dataset = TrajGraderDataset(file_root="./datasets/balance_game/diffusion_reward/test")
    # INFO: Initialize dataloader
    train_data_loader = DataLoader(train_dataset, batch_size=128, num_workers=1, shuffle=True, pin_memory=True)
    valid_data_loader = DataLoader(valid_dataset, batch_size=64, num_workers=1, shuffle=True, pin_memory=True)
    test_data_loader = DataLoader(test_dataset, batch_size=64, num_workers=1, shuffle=True, pin_memory=True)
    # INFO: Initialize log dir
    log_dir = Path("./logs/marl/20240108-212645") / "grader_log"
    ...
    # INFO: We only consider the trajectory in the grader Grader_FC
    global_traj_grader = Grader_FC(input_dim=20, out_dim=1, hidden_dim=128, constrain_out=True)
    grader_optimizer = torch.optim.Adam(global_traj_grader.parameters(), lr=1e-3) # original: 1e-3
    ...
    for ep in tqdm(range(epoch_num)):
        ...
        for batch in train_data_loader:
            ...
            traj, rew = batch
            # batch_size, traj_pt_num, traj_pt_dim = traj.shape
            # INFO: We consider data trajectory but also the previous red detection of blues
            train_loss = global_traj_grader.compute_loss(traj, rew, loss_func=torch.nn.L1Loss()) # MSELoss
            grader_optimizer.zero_grad()
            train_loss.backward()
            if ep != 0:
                grader_optimizer.step()
            episode_train_loss.append(train_loss.item())
        ...
        if ep % 2 == 0:
            for batch in valid_data_loader:
                ...
                traj, rew = batch
                # INFO: We consider data trajectory but also the previous red detection of blues
                # valid_loss = global_traj_grader.compute_loss(global_dict["detections"], traj, dis_cum_rew(rew), loss_func=torch.nn.L1Loss())
                valid_loss = global_traj_grader.compute_loss(traj, rew, loss_func=torch.nn.L1Loss())
                episode_valid_loss.append(valid_loss.item())
            for batch in test_data_loader:
                # INFO: batch is composed of red trajectories, {hideout, start loc}, start loc, rewards
                traj, rew = batch
                # INFO: We consider data trajectory but also the previous red detection of blues
                # eval_loss = global_traj_grader.compute_loss(global_dict["detections"], traj, dis_cum_rew(rew), loss_func=torch.nn.L1Loss())
                eval_loss = global_traj_grader.compute_loss(traj, rew, loss_func=torch.nn.L1Loss())
                episode_eval_loss.append(eval_loss.item())    

            episode_valid_ave_loss = np.array(episode_valid_loss).mean()
            episode_eval_ave_loss = np.array(episode_eval_loss).mean()     

            if episode_valid_ave_loss < min_valid_loss:
                torch.save(global_traj_grader, log_dir/"best.pth")
                min_valid_loss = episode_valid_ave_loss
            else:
                pass

        episode_train_ave_loss = np.array(episode_train_loss).mean()


        writer.add_scalars('TrajGraderLoss', {"train": episode_train_ave_loss, "valid": episode_valid_ave_loss, "eval": episode_eval_ave_loss}, ep)
