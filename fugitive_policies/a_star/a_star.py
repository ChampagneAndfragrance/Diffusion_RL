...
def a_star(start_m, goal_m, gmap, movement='8N', occupancy_cost_factor=25):
    """
    ...
    gmap.reset_visited() # clear visited flags
    ...
    if gmap.is_occupied_idx(start):
        # if start node is occupied, sample random points around it until we find a free one
        while True:
            start = gmap.get_index_from_coordinates(start_m[0] + random.randint(-3, 3), start_m[1] + random.randint(-3, 3))
            if not gmap.is_occupied_idx(start):
                break
        path = []
    else:
        path = []
