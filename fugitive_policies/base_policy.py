...
class Observation:
    ...
    def process_observation(self, observations):
        """ This is now used to process FUGITIVE observation! """
        self.reset()
        ...
        # INFO: If red can see the blue team
        for _ in range(self.num_helicopters):
            detected = 1 if np.linalg.norm(observations[start:start+2] * np.array([DIM_X, DIM_Y])) < 2000 else 0
            heli = Heli(detected, observations[start:start+2] + self.location / np.array([DIM_X, DIM_Y]))
            self.heli_list.append(heli)
            start += 4

        for _ in range(self.num_search_parties):
            detected = 1 if np.linalg.norm(observations[start:start+2] * np.array([DIM_X, DIM_Y])) < 1000 else 0
            search_party = SearchParty(detected, observations[start:start+2] + self.location / np.array([DIM_X, DIM_Y]))
            self.search_party_list.append(search_party)
            start += 4

        # INFO: These two chunks of code are for fugitive_detection_of_parties
        # for _ in range(self.num_helicopters):
