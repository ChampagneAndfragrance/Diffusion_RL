...
class RRTStarAdversarialAvoid(RRTStarAdversarial):
    ...
    def reset(self):
        ...
        # Add the closest location as the goal
        # self.observations.process_observation(self.env._fugitive_observation)
        # _, closest_unknown_hideout = self.get_closest_hideouts(self.observations.location)
        # self.env.goal_location = closest_unknown_hideout.location

    def action_to_different_unknown_hideout(self, current_goal):
        hideout_distances = {}
        for hideout in self.observations.unknown_hideout_list:
            """When there are multiple unknown hideouts"""
            if len(self.observations.unknown_hideout_list) != 1:
        ...
        # add goal location to the environment
        self.env.goal_location = hideout.location
    ...
    def straight_line_action_to_hideout(self, hideout):
        if hideout == None:
            return self.straight_line_action_to_closest_unknown_hideout()
        theta = self.calculate_desired_heading(self.observations.location, hideout.location)
        dist = distance(self.observations.location, hideout.location)
        speed = np.clip(dist, 0, 7.5)
        self.current_hideout_goal = hideout
        return np.array([speed, theta], dtype=np.float32)
    ...
    def action_to_closest_unknown_hideout(self, plot=False):
        ...
        if len(self.actions) == 0:
            # closest_known_hideout and closest_unknown_hideout are Hideout classes with property 'location' as np arrays
            ...
            # add goal location to the environment
            self.env.goal_location = goal_location
    ...
    def action_to_random_hideout(self, plot=False):
        ...
        if mountain_in_range:
            # if we're within a mountain, don't use rrt*
            return self.wrapper_avoid_mountain(self.straight_line_action_to_hideout(self.current_hideout_goal))
        ...
        if len(self.actions) == 0:
            ...
            # INFO: Try to smooth the RRT star paths with spline functions
            downsample_ratio = 4
            path_x = np.array(path)[::downsample_ratio,0].tolist()
            path_x[-1] = path[-1][0]
            path_y = np.array(path)[::downsample_ratio,1].tolist()
            path_y[-1] = path[-1][1]
            sp = Spline2D(path_x, path_y, kind='cubic')
            # s = np.arange(0, sp.s[-1], 50)
            s = np.linspace(0, sp.s[-1], 10).astype(int)
            rx, ry = [], []
            for i_s in s:
                ix, iy = sp.calc_position(i_s)
                rx.append(ix)
                ry.append(iy)
            rx = np.array(rx).astype(int)
            rx[0], rx[-1] = path[0][0], path[-1][0]
            ry = np.array(ry).astype(int)
            ry[0], ry[-1] = path[0][-1], path[-1][-1]
            # self.path = path
            print("Planning time:", time.time() - start)
            startpos = (self.s_start.x, self.s_start.y)
            endpos = (self.s_goal.x, self.s_goal.y)
            if plot:
                ...
                plotter.create_path(path)
                # plt.savefig(f'figures/path_rrt_star_{self.terrain_cost_coef}.png', dpi=600)
                plt.savefig('figures/path_rrt_star_%s_original.png' % timestr, dpi=600)
                plt.close()

                # INFO: draw the smoothed paths
                plt.plot(rx, ry, "-")
                plt.xlim(0, 2428)
                plt.ylim(0, 2428)
                plt.savefig('figures/path_rrt_star_%s_spline.png' % timestr, dpi=600)
    ...
    def predict(self, observation, plot=True):
        return (self.get_desired_action(observation), None)
    
class RRTStarOnly(RRTStarAdversarialAvoid):
    def __init__(self, env,             
            n_iter=1500, 
            step_len=150, 
            search_radius=150, 
    ...
    def get_desired_action(self, observation, plot=True):
        self.observations.process_observation(observation)
        if self.DEBUG:
            print('t=', self.env.timesteps)
            print('fugitive current location', self.observations.location)

        desired_action = self.action_to_random_hideout(plot=plot)
        # import time
        # time.sleep(1)
        if self.DEBUG:
            print('---------------------------')
    ...
    def global_plan(self, downsample_ratio, waypt_num, observation):
        self.observations.process_observation(observation)
        # closest_known_hideout and closest_unknown_hideout are Hideout classes with property 'location' as np arrays
        random_hideout = self.get_random_hideout()
        goal_location = random_hideout.location
        ...
        sp = Spline2D(path_x, path_y, kind='cubic')
        # s = np.arange(0, sp.s[-1], 50)
        s = np.linspace(0, sp.s[-1], waypt_num).astype(int)
        rx, ry = [], []
        for i_s in s:
            ix, iy = sp.calc_position(i_s)
            rx.append(ix)
