...
class dwa_local_planner(object):
    def __init__(self, max_speed=15.0, min_speed=0, max_yaw_rate=math.pi, max_accel=3.0, max_delta_yaw_rate=np.inf, 
                    v_resolution=0.1, yaw_rate_resolution=0.15*math.pi, dt=1, predict_time=10.0, to_goal_cost_gain=0.25, 
                        speed_cost_gain=0.1, globalPath_cost_gain=0.006, obstacle_cost_gain=100, robot_stuck_flag_cons=0.001) -> None:
        # robot parameter
    ...
    def add_to_obstacle(self, obstacles, obstacle_id_seen):
        for obstacle, obstacle_id in zip(obstacles, obstacle_id_seen):
            # INFO: Process the dynamic agents
            if obstacle_id < 3:
                self.ob_hc_sp[obstacle_id] = obstacle
            # INFO: Process static cams and mountain
            elif obstacle_id not in self.ob_id:
    ...
    def dwa_control(self, x, goal, global_path):
        """
    ...
    def calc_control_and_trajectory(self, x, dw, goal, global_path):
        ...
        dynamic_static_obstacles_xy = self.ob_hc_sp + self.ob_cam 
        dynamic_static_obstacles_xy = [ele for ele in dynamic_static_obstacles_xy if ele != []]
        ...
        for v in np.arange(dw[0], dw[1], self.v_resolution):
            for y in np.arange(dw[2], dw[3], self.yaw_rate_resolution):
                ...
                ob_cost = 0 if len(dynamic_static_obstacles_xy)==0 else self.obstacle_cost_gain * self.calc_obstacle_cost(trajectory, np.array(dynamic_static_obstacles_xy))
        ...
        print("The final to_goal_cost, speed_cost, ob_cost, to_global_path_cost are: \n ", to_goal_cost , "\n", speed_cost , 
                                                                                        "\n", ob_cost , "\n", to_global_path_cost , "\n")
