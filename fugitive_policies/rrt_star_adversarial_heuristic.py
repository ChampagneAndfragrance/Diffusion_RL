...
import math
from fugitive_policies.base_policy import Observation, DetectionObject, Camera, Heli, SearchParty, Hideout
...
class RRTStarAdversarial:
    def __init__(self, env,             
        ...
        self.vertex_pos = np.ones((10000, 2)) * np.inf # Initialize more than possible just in case
    ...
    def get_closest_hideouts(self, location):
        closest_known_hideout, _ = self.get_closest_hideout(location, self.env.known_hideout_list)
        closest_unknown_hideout, _ = self.get_closest_hideout(location, self.env.unknown_hideout_list)

        return closest_known_hideout, closest_unknown_hideout

    def get_random_hideout(self):
        hideout_num = len(self.env.hideout_list)
        hideout_idx = np.random.choice(hideout_num, 1)[0]
        random_hideout = self.env.hideout_list[hideout_idx]
        return random_hideout
    ...
    def check_collision_mountain(self, start_node, end_node):
        ...
        for mountain_location in self.terrain.mountain_locations:
            ...
            delta = a - b
            unit_vector = delta / np.linalg.norm(delta + 1e-6)
            # print("delta = ", delta)
            d = np.cross(c - a, unit_vector)
            if -MOUNTAIN_OUTER_RANGE < d <  MOUNTAIN_OUTER_RANGE:
                return True

            if unit_vector @ a < unit_vector @ c < unit_vector @ b: # Since Python 3.5, it’s been possible to use @ to multiply matrices
                return True
    ...
    def find_near_neighbor(self, node_new):
        ...
        if len(d) == 1 and len(d[0]) == 0:
            ...
            indices = np.concatenate(d) # indices is now an np array (converted from tuple containing arrays)
    ...
    def terrain_cost_path(self, start_node, end_node):
        ...
        # terrain_points: point locations in the local frame with start_node as the origin, shape: (num_points, 2)
        terrain_points = np.einsum('ij,ik->ij', points, range_points) # einsum: http://www.atyun.com/32288.html (language: CHN)
        indices = np.round(terrain_points).astype('int') # round location to pixel?
        arr = self.terrain.world_representation[1, :, :] # load the costmap?
        xs = indices[:, 0] + start_node.x # x coordinate in global frame
        ys = indices[:, 1] + start_node.y # y coordinate in global frame
    ...
    def plan(self, endpos):
        ...
        while k < self.iter_max or self.goal_not_found:
            ...
            if self.check_collision_mountain(node_near, node_new): # Check if the straight line path between the start and end location pass through the mountain.
                continue

            if node_new and not self.check_collision_mountain(node_near, node_new):
                node_new.parent = node_near # the parent of the new node is the existing nearest node
                node_near.children.add(node_new) # the children of the existing nearest node will include the new node
    ...
    def get_closest_hideout_loc(self, observation):
        self.observations.process_observation(observation)
