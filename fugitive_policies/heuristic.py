...
from fugitive_policies.cache_evasive_angles import pool2d
...
class HeuristicPolicy:
    def __init__(self, env, random_mountain=False, mountain_travel="optimal", epsilon=0, max_speed=7.5):
        """ 
        ...
        self.DEBUG = False
        self.initialize_evasive_planner()
    ...
    def initialize_evasive_planner(self):
        array = self.terrain.forest_density_array
        array = (array < 0.4) * 1.0
        array_pool = pool2d(array, 6, 1, pool_mode='avg') * 36
        array_thresh = (array_pool > 18) * 255
        nonzero = cv2.findNonZero(array_thresh)
        self.dark_forest_cache = nonzero

    def calculate_evasive_angle(self, current_location, hideouts):
        nonzero = self.dark_forest_cache
        ...
        if distanced[nearest_index] > 500 * (2 ** 1/2):
            candidate =  (1500,1500)
        else:
            candidate = (nonzero[nearest_index] + 3)[0]

        # print(f"evasive angle: {current_location}, {candidate}")
        angle = self.calculate_desired_heading(current_location, candidate)
        return angle
    ...
    def get_desired_action(self, observation):
        ...
        if camera_in_range and detected_helicopter and detected_search_party:
            ...
            theta = self.calculate_desired_heading(self.observations.location, closest_known_hideout.location)
            desired_action = np.array([self.max_speed, theta], dtype=np.float32)
            ...
            if distance(self.observations.location, detected_helicopter.location) > 100:
                ...
                if in_dense_forest and self.current_behavior == self.behaviors[0]:
                    ...
                    if self.last_action == 'you have almost evaded, slow down, change direction':
                        desired_action = np.array([self.max_speed, self.current_behavior_heading], dtype=np.float32)
        ...
        #     desired_action = np.array([self.max_speed, desired_heading], dtype=np.float32)
        else:
            if self.DEBUG:
    ...
    def action_to_closest_known_hideout(self):
        ...
        speed = np.clip(dist, 0, self.max_speed)
