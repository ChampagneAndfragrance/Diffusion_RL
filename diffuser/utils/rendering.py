...
class PrisonerRenderer:
    def __init__(self, background=True):
        ...
        if self.background:
            ...
            self._background = imageio.imread('diffuser/utils/iros_env.png')
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        if conditions is not None:
            if len(conditions[1]) > 0:
                plt.scatter((conditions[1][:,1] + 1) * (2428/2), (conditions[1][:,0] + 1) * (2428/2) , c='red', zorder=30, marker='x')
    ...
    def composite_samples_gt(self, savepath, samples, gt_path, title=None):
        plt.clf()
        fig = plt.gcf()
        fig.set_size_inches(5, 5)
        # plt.imshow(self._background * .5,
        ...
        for sample in samples:
            plt.plot(sample[:,1], sample[:,0], c='black', zorder=10)
            # plt.scatter(observations[:,1], observations[:,0], c=colors, zorder=20)

        plt.plot(gt_path[0, : ,1], gt_path[0, : ,0], c='red', zorder=20)
...
class PrisonerRendererGlobe:
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        size = 1
        colors = plt.cm.jet(np.linspace(0,1,path_length))
        x, y = m(observations[:, 0], observations[:, 1])
    ...
    def composite(self, savepath, paths, ncol=5, **kwargs):
        ...
        for path, kw in zipkw(paths, **kwargs):
            img = self.renders(*path, **kw)
            images.append(img)
            # imageio.imsave(os.path.join(savepath, f'{i}.png'), img)
            i += 1
        images = np.stack(images, axis=0)
        ...
        imageio.imsave(savepath, images)
        # composite_path = os.path.join(savepath, 'composite.png')
        # imageio.imsave(composite_path, images)
        print(f'Saved {len(paths)} samples to: {savepath}')
    ...
    def composite_samples_gt(self, savepath, samples, gt_path, title=None):
        ...
        size = 1
        for sample in samples:
            # path_length = len(sample)
            # colors = plt.cm.viridis(np.linspace(0,1,path_length))
            x, y = m(sample[:, 0], sample[:, 1])
            # m.scatter(x, y, s=size, c=colors, marker='o')
            m.plot(x, y, linestyle='-', c='yellow', label='Path', zorder=10)
        
        # path_length = len(gt_path[0])
        # colors = plt.cm.jet(np.linspace(0,1,path_length))
        x, y = m(gt_path[0, : ,0], gt_path[0, : ,1])
        # m.scatter(x, y, s=size, c=colors, marker='o')
        m.plot(x, y, linestyle='-', c='red', zorder=20)
