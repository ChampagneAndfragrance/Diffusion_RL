...
class MovingObject(AbstractObject):
    def __init__(self, terrain, location, speed):
        ...
        self.planned_path = []
        self.debug = False
        self.normalized_vel = np.zeros((2))
        self.step_dist_xy = np.zeros((2))
    ...
    def get_action_according_to_plan(self):
        ...
        if current_movement[0] == 'l':
            ...
            if distance(self.location, (current_movement[1], current_movement[2])) <= 10:
                action = (0, 0, speed) # newly added
                self.planned_path.pop(0)
                if len(self.planned_path) == 0:
                    # move to loc just finished, start a spiral
                    self.plan_spiral()
                    # self.plan_path_to_stop()
    ...
    def get_action_according_to_plan_para(self):
        ...
        if current_movement[0] == 'l':
            # self.path_v2(destination=(current_movement[1], current_movement[2]))
            direction = self.transform_destination_to_direction(np.array([current_movement[1], current_movement[2]]))
            speed = min(distance(self.location, [current_movement[1], current_movement[2]]), self.speed, current_movement[3])
            action = (direction[0], direction[1], speed)
            if distance(self.location, (current_movement[1], current_movement[2])) <= 10:
                # print("see here.")
                action = (0, 0, speed) # newly added
                ...
                if len(self.planned_path) == 0:
                    # move to loc just finished, start a spiral
                    self.plan_spiral_para(speed_ratio=0.1)
                    # self.plan_path_to_stop()
        elif current_movement[0] == 'ls':
            # self.path_v2(destination=(current_movement[1], current_movement[2]),
            #              constraint_speed=30, mountain_outer_range=150)  # spiral detection range=22, and make mountain behavior more conservative
            direction = self.transform_destination_to_direction(np.array([current_movement[1], current_movement[2]]))
            speed = min(distance(self.location, [current_movement[1], current_movement[2]]), current_movement[3])
            action = (direction[0], direction[1], speed)
            if distance(self.location, (current_movement[1], current_movement[2])) <= 10:
                self.planned_path.pop(0)
                if len(self.planned_path) == 0:
            ...
            if current_movement[3] > 0:
                ...
                # success = self.path_v2(direction=(current_movement[1], current_movement[2])) 
                # TODO: Sean - Is success always true?
                action = (current_movement[1], current_movement[2], current_movement[4])
                # if success: 
                self.planned_path[0] = (current_movement[0], current_movement[1], current_movement[2], current_movement[3] - 1, current_movement[4], current_movement[5])
                # print("self.planned_path[0]: ", self.planned_path[0])
            else:
                action = (current_movement[1], current_movement[2], current_movement[5])
                # print("action = ", action)
                # # slow moving mode to detect carefully
                # if current_movement[4] == 'c':
                #     # chasing
    ...
    def plan_spiral_para(self, speed_ratio):
        detection_range = 22  # 100% PoD of SP when in wood and fugitive is fast walking (speed=7.5)
        spiral_unit = 2 * detection_range
        directions_of_spiral = np.array([
            [1, 0],
        ...
        while current_spiral_coefficient < 8:
            ...
            if self.terrain.violate_edge_constraints(next_loc[0], next_loc[1], 1, 1):
                break
            self.planned_path.append(('ls', next_loc[0], next_loc[1], speed_ratio * self.speed))
            index_of_direction = (index_of_direction + 1) % 4
            if index_of_direction == 0 or index_of_direction == 2:
                current_spiral_coefficient += 1
            current_loc = next_loc
    ...
    def plan_path_to_random_para(self, speed_ratio):
        sampled_destination = self.sample_destinations(sample_method='random')
        self.plan_path_to_loc_para(speed_ratio=1.0, loc=sampled_destination)

    def plan_path_to_stop(self):
        sampled_destination = self.location
        self.plan_path_to_loc(sampled_destination)       

    def plan_path_to_stop_para(self):
        sampled_destination = self.location
        self.plan_path_to_loc_para(speed_ratio=0, loc=sampled_destination)  
    ...
    def plan_pid_para(self, P, trgt_loc):
        curr_loc = np.array(self.location)
        speed = np.linalg.norm(P * (trgt_loc - curr_loc))
        self.planned_path = [('l', trgt_loc[0], trgt_loc[1], speed)]

    def plan_path_to_intercept_para(self, speed_diff_ratio, speed, direction, current_loc):
        """
        Determine the path the helicopter or search party will take to intercept the fugitive
        Note this planning does not account for mountains
            :param speed: the speed of the fugitive calculated
        ...
        distance_movement = np.sqrt(path_vector[0] ** 2 + path_vector[1] ** 2)
        if distance_movement < self.speed:
            self.planned_path = [('l', current_loc[0], current_loc[1], self.speed)]
            return
        ...
        if is_blue_on_fugitive_movement_side:
            if self.terrain.violate_edge_constraints(x_tangent, y_tangent, 1, 1):
                ...
                if time_to_tangent_fugitive < time_to_tangent_blue:
                    # fugitive arrives at the tangent point earlier than the blue
                    # go to the tangent point and follow the fugitive's direction
                    self.plan_path_to_loc_para(speed_ratio=1.0, loc=[x_tangent, y_tangent])
                    # the distance between blue and red when the blue arrives the tangent point
                    distance_to_go = speed * time_to_tangent_blue - distance(current_loc, tangent_point)
                    buffer_distance = 30
                    time_to_buffer = (distance_to_go - buffer_distance) / (self.speed - speed)
                    speed_diff = speed_diff_ratio * (self.speed + speed)
                    blue_speed = speed_diff + speed
                    if time_to_buffer > 1:
                        # could have a few steps of fast movements
                        self.planned_path.append(('d', np.cos(direction), np.sin(direction), time_to_buffer - 1, self.speed, blue_speed))
                    else:
                        # just use constrained speed
                        self.planned_path.append(('d', np.cos(direction), np.sin(direction), -1, self.speed, blue_speed))
                else:
                    # fugitive arrives at the tangent point later than the blue
                    # go to the tangent point and follow the opposite of fugitive's direction
                    self.plan_path_to_loc_para(speed_ratio=1.0, loc=[x_tangent, y_tangent])
                    ...
                    time_to_buffer = (distance_to_go - buffer_distance) / (self.speed + speed)
                    # if self.speed == 127:
                        # print("time_to_buffer = ", time_to_buffer)
                    speed_diff = speed_diff_ratio * (self.speed + speed)
                    blue_speed = speed_diff - speed
                    if time_to_buffer > 1:
                        # could have a few steps of fast movements
...
class DetectionObject(AbstractObject):
    ...
    def detect(self, location_object, speed_object, normalized_timestep, blue_id=None, prisoner_loc=None):
        ...
        if blue_id is not None:
            self.detection_ranges[blue_id] = 3 * base_100_pod_distance
        else:
            self.detection_ranges[0] = 3 * base_100_pod_distance

        if distance < base_100_pod_distance:  # the PoD is 100% within base_100_pod_distance
            self.last_two_detections.pop(0)  # Remove old detection
            self.last_two_detections.append([normalized_timestep,
                        location_object[0]/2428, 
                        location_object[1]/2428])  # Append latest detection
            if prisoner_loc is None:
                return [1, location_object[0]/2428, location_object[1]/2428]
            else:
                return [1, (location_object[0]-prisoner_loc[0])/2428, (location_object[1]-prisoner_loc[1])/2428]

        if distance > base_100_pod_distance * 3:  # the PoD is 0% outside 3*base_100_pod_distance
            return [0, -1, -1]
        # INFO: Not Deterministic Red Detection of Blue
        ...
        # INFO: Deterministic Red Detection of Blue
