...
def create_observation_space_ground_truth(num_known_cameras, num_unknown_cameras, num_known_hideouts, num_unknown_hideouts, num_helicopters, num_search_parties, terrain_size=0):
        ...
        for i in range(num_helicopters):
            ...
            obs_names.add_name('helicopter_detect_%d'%i, 4)
        for i in range(num_search_parties):
            observation_low.extend([0])
            observation_high.extend([1])
            obs_names.add_name('search_party_detect_%d'%i, 4)
        for i in range(num_known_cameras+num_unknown_cameras):
            observation_low.extend([0])
            observation_high.extend([1])
            obs_names.add_name('camera_detect_%d'%i, 4)
...
def create_observation_space_fugitive(num_known_cameras, num_unknown_cameras, num_known_hideouts, num_unknown_hideouts, num_helicopters, num_search_parties, terrain_size=0):
    ...
    for i in range(num_known_hideouts+num_unknown_hideouts):
        ...
        obs_names.add_name('hideout_known_%d'%i, 1)
        obs_names.add_name('hideout_loc_%d'%i, 2)
    # 2.3 mountain locations
    observation_low.extend([0, 0])
    observation_high.extend([1, 1])
    obs_names.add_name('mountain_loc', 2)
    # 3. self location (divided by 2428), self speed (divided by 15), self heading (divided by pi)
    observation_low.extend([0, 0, 1.0 / 15, -1])
    observation_high.extend([1, 1, 1, 1])
    obs_names.add_name('prisoner_loc', 2)
    obs_names.add_name('prev_action', 2)
    # 4. detection of [helicopters and its id (0), search parties and id (1), cameras and id (-1)]
    # Detection is encoded by a four tuple [id, b, x, y] where b in binary. If b=1 (detected), [x, y] will have the detected location in world coordinates. If b=0 (not detected), [x, y] will be [-1, -1].
    # for i in range(num_helicopters):
    #     observation_low.extend([-1, 0, -1, -1, -1, -1])
    #     observation_high.extend([1, 1, 1, 1, 1, 1])
    #     obs_names.add_name('helicopter_detect_%d'%i, 6)
    # for i in range(num_search_parties):
    #     observation_low.extend([-1, 0, -1, -1, -1, -1])
    #     observation_high.extend([1, 1, 1, 1, 1, 1])
    #     obs_names.add_name('search_party_detect_%d'%i, 6)
    # for i in range(num_known_cameras+num_unknown_cameras):
    #     observation_low.extend([-1, 0, -1, -1])
    #     observation_high.extend([1, 1, 1, 1])
    #     obs_names.add_name('camera_detect_%d'%i, 4)
    # Predicted blue shape
    for i in range(num_helicopters+num_search_parties):
        observation_low.extend([-1, -1, -1, -1])
        observation_high.extend([1, 1, 1, 1])
        obs_names.add_name('blue_predicted_%d'%i, 4)
    # Terrain shape
...
def create_observation_space_prediction(num_known_cameras, num_unknown_cameras, num_known_hideouts, num_helicopters, num_search_parties, terrain_size):
    # observation and action spaces (for fugitive)
    obs_names = ObservationNames()
    # observation space contains
    # 1. time (divided by 4320 to normalize)
...
def create_observation_space_blue_team(num_known_cameras, num_unknown_cameras, num_known_hideouts, num_helicopters, num_search_parties, terrain_size=0, include_start_location_blue_obs=False,
        include_fugitive_location_in_blue_obs=True):
        ...
        # 5. Include the predicted location of the prisoner
        observation_low.extend([-1, -1])
        observation_high.extend([1, 1])
        obs_names.add_name('prisoner_predicted_loc', 2)
        # Terrain shape
        observation_low.extend([0]*terrain_size)
        observation_high.extend([1]*terrain_size)
        ...
        if include_start_location_blue_obs:
            observation_low.extend([0, 0])
            observation_high.extend([1, 1])
            obs_names.add_name('prisoner_starting', 2)
...
def create_observation_space_blue_team_original(num_known_cameras, num_unknown_cameras, num_known_hideouts, num_helicopters, num_search_parties, terrain_size=0, include_start_location_blue_obs=False,
...
def transform_predicted_detection_of_fugitive(parties_detection_of_fugitive, predicted_locations_from_last_two_detections):
    """ This is used to remove the repeated detections of the fugitive in the blue parties observation space.
    ...
    """
    one_hot = parties_detection_of_fugitive[::3]
    return one_hot + predicted_locations_from_last_two_detections
    # # if not any(one_hot):
    # #     return one_hot + [-1, -1]
    # # else:
    # #     index = one_hot.index(1)
