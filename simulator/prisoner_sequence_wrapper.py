...
class BlueSequenceEnv(gym.Wrapper):
    """ Batches the observations for an lstm """
    def __init__(self,
                 env: PrisonerEnv,
                 sequence_len=1):
        ...
        # self.prediction_observation_array = np.zeros((sequence_len,) + self.env.prediction_observation_space.shape)
    def reset(self):
        obs, partial_obs = self.env.reset()
        self.observation_array = np.zeros((self.sequence_len,) + self.observation_space.shape)
        self.observation_array[-1] = obs
        return self.observation_array, partial_obs

    def step(self, action):
        obs, partial_obs, reward, done, i = self.env.step(action)
        self.observation_array[0:-1] = self.observation_array[1:]
        self.observation_array[-1] = obs
        ...
        return self.observation_array, partial_obs, reward, done, i


class RedSequenceEnv(gym.Wrapper):
    """ Batches the observations for an lstm """
    def __init__(self,
                 env: PrisonerEnv,
                 sequence_len=1):
    ...
    def step(self, action):
        obs, reward, done, i, partial_obs = self.env.step(action)
        self.observation_array[0:-1] = self.observation_array[1:]
        self.observation_array[-1] = obs
